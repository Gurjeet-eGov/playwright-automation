...
@pytest.mark.ui
@pytest.mark.localization
def test_pgr_dashboard_rev(page_chr):
    page = page_chr
    ...
    # Find localization leaks
    loc_codes = helpers.find_loc_codes(locales, isTable=True)
    helpers.write_json(loc_codes, 'test_pgr_dashboard_rev.json')
    expect(table_row).to_be_visible()

@pytest.mark.ui
@pytest.mark.localization
def test_pgr_dashboard_rev_tb(page_chr):
    page = page_chr
    page.goto(BASE_URL + '/dashboard/pgr')
    page.wait_for_load_state("networkidle")
    body = page.locator("#divToPrint")
    ...
    table_data = re.split(r'[\t\n]', table_data)
    table_data = helpers.find_loc_codes(table_data)
    helpers.write_json(table_data, 'test_pgr_dashboard_rev_tb.json')
    assert True
